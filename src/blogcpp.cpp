...
// this function is called from the worher threads. It must never throw any
// exceptions or c++ calls std::terminate
void collectPostData(string inputfile, bool is_page) {
	...
	{
		cout << "Sorry: collectPostData " << ex.what() << endl;
	}
	catch (...)
	{
		cout << "Sorry: collectPostData unknown exception" << endl;
	}
}
...
void renderIndex(json extradata, stringstream& ss_ret) {
    ...
    for (int k = 1; k <= total_pages; k++) {
        ...
        int offset = (k == 1 ? 0 : cfgs.cfg_maxitems * (k - 1)); // The second page starts with the (1 * maxitems)th post.
        ...
        if (total_pages == 1) {
            ...
            posts_per_page = static_cast<int>(ceil(static_cast<float>(vsi_posts_with_sticky.size()) / cfgs.cfg_maxitems) / k);
            vector<SingleItem> vec{vsi_posts_with_sticky.begin() + offset, next(vsi_posts_with_sticky.begin() + offset, posts_per_page)};
            v_newest_x = vec;
        }
...
int main(int argc, char* argv[]) {
    ...
    // This can throw too if the blog.cfg file is not there
    try {
